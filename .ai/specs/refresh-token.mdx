...
- **Admin tokens excluded**: Admin-generated SDK tokens do NOT receive refresh tokens
...
Two authentication methods with different refresh token behavior:

| Method | Auth | Refresh Token |
|--------|------|---------------|
| App credentials | `app_id` + `app_secret` in body | Yes |
| Admin auth | Bearer token (developer) | No |

<Note>
**Why no refresh token for admin-generated tokens?**

Admin tokens are for testing and debugging. They should be short-lived and require re-authentication via the admin dashboard. This prevents long-lived tokens from being accidentally distributed.
</Note>
