...
- known_gap:                 Nothing reaps stale `sync:status:run:*` records. `gc_stuck_backfills` runs every 3 minutes but only scans `garmin:backfill:*:lock`. Until a reaper exists, an orphaned run stays visible until its TTL. **Do not build one** — upstream PR #1448 (`SyncRun` tracking) adds `close_stale_sync_runs_task.py` with a Postgres-backed run table, a dedicated `STALE` status distinct from `FAILED`, a Redis liveness pre-check so a slow backfill is not closed, and `SYNC_RUN_STALE_AFTER_HOURS` config. A fork reaper was written on 2026-08-30 and then deleted on discovering it: it duplicated #1448 and was strictly worse. Take the gap on merge.
