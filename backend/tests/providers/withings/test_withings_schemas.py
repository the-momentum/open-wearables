...
def test_resolve_window_from_startdate_enddate() -> None:
    n = WithingsNotification(userid="1", appli=44, startdate=1728000000, enddate=1728003600)
    window = n.resolve_window()
    assert window == (
        datetime.fromtimestamp(1728000000, tz=timezone.utc),
        datetime.fromtimestamp(1728003600, tz=timezone.utc),
    )


def test_resolve_window_from_single_date_expands_to_day() -> None:
    # appli 16 (activity) sends a single `date=YYYY-MM-DD`, not a start/end range.
    n = WithingsNotification(userid="1", appli=16, date="2018-07-02")
    window = n.resolve_window()
    assert window == (
        datetime(2018, 7, 2, tzinfo=timezone.utc),
        datetime(2018, 7, 3, tzinfo=timezone.utc),
    )


def test_resolve_window_none_for_profile_change() -> None:
    n = WithingsNotification(userid="1", appli=46, action="update")
    assert n.resolve_window() is None


def test_resolve_window_garbage_date_returns_none() -> None:
    n = WithingsNotification(userid="1", appli=16, date="not-a-date")
    assert n.resolve_window() is None
