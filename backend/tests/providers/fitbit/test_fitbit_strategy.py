...
def test_fitbit_strategy_oauth_is_correct_type():
    strategy = FitbitStrategy()
    assert isinstance(strategy.oauth, FitbitOAuth)


def test_fitbit_strategy_workouts_is_correct_type():
    strategy = FitbitStrategy()
    assert isinstance(strategy.workouts, FitbitWorkouts)


def test_fitbit_strategy_oauth_shares_provider_name():
    strategy = FitbitStrategy()
    assert strategy.oauth.provider_name == "fitbit"


def test_fitbit_strategy_workouts_shares_oauth_instance():
    strategy = FitbitStrategy()
    assert strategy.workouts.oauth is strategy.oauth


def test_fitbit_strategy_repositories():
    strategy = FitbitStrategy()
