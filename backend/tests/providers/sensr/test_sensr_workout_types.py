...
def test_sensr_workout_type_from_likely_name() -> None:
    assert get_unified_workout_type("running") == WorkoutType.RUNNING


def test_sensr_workout_type_from_type_fallback() -> None:
    assert get_unified_workout_type(None, "cycling") == WorkoutType.CYCLING


def test_sensr_workout_type_unknown_defaults_to_other() -> None:
