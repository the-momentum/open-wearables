...
DELETE /api/v1/users/{user_id}/events/sleep/{id} and
DELETE /api/v1/users/{user_id}/events/meals/{id} endpoints including:
...
from app.models import EventRecord, MealDetails, SleepDetails, WorkoutDetails
...
class TestDeleteMeal:
    """Test suite for DELETE /users/{user_id}/events/meals/{meal_id}."""

    def test_delete_meal_success(self, client: TestClient, db: Session) -> None:
        user = UserFactory()
        ds = DataSourceFactory(user=user)
        meal = EventRecordFactory(mapping=ds, category="meal")
        MealDetailsFactory(event_record=meal)
        api_key = ApiKeyFactory()

        response = client.delete(
            f"/api/v1/users/{user.id}/events/meals/{meal.id}",
            headers=api_key_headers(api_key.plain_key),
        )

        assert response.status_code == 204
        assert db.get(EventRecord, meal.id) is None

    def test_delete_meal_cascades_details(self, client: TestClient, db: Session) -> None:
        user = UserFactory()
        ds = DataSourceFactory(user=user)
        meal = EventRecordFactory(mapping=ds, category="meal")
        detail = MealDetailsFactory(event_record=meal)
        detail_record_id = detail.record_id
        api_key = ApiKeyFactory()
        ...
        assert response.status_code == 204
        assert db.get(MealDetails, detail_record_id) is None

    def test_delete_meal_not_found(self, client: TestClient, db: Session) -> None:
        user = UserFactory()
        api_key = ApiKeyFactory()

        response = client.delete(
            f"/api/v1/users/{user.id}/events/meals/{uuid4()}",
            headers=api_key_headers(api_key.plain_key),
        )

        assert response.status_code == 404

    def test_delete_meal_wrong_user(self, client: TestClient, db: Session) -> None:
        owner = UserFactory()
        other_user = UserFactory()
        ds = DataSourceFactory(user=owner)
        meal = EventRecordFactory(mapping=ds, category="meal")
        ...
        response = client.delete(
            f"/api/v1/users/{other_user.id}/events/meals/{meal.id}",
            headers=api_key_headers(api_key.plain_key),
        )

        assert response.status_code == 404
        assert db.get(EventRecord, meal.id) is not None

    def test_delete_meal_wrong_category(self, client: TestClient, db: Session) -> None:
        """Deleting a workout record via meals endpoint should return 404."""
        user = UserFactory()
        ds = DataSourceFactory(user=user)
        workout = EventRecordFactory(mapping=ds, category="workout", type_="running")
        api_key = ApiKeyFactory()
        ...
        response = client.delete(
            f"/api/v1/users/{user.id}/events/meals/{workout.id}",
            headers=api_key_headers(api_key.plain_key),
        )
