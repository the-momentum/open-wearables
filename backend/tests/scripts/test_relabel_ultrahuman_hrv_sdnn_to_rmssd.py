"""Tests for the one-off data migration that relabels Ultrahuman HRV SDNN -> RMSSD.

Ultrahuman reports RMSSD-based HRV, but the ingestion historically stored it under the
SDNN series type (id=3). This script relabels existing Ultrahuman rows to RMSSD (id=7),
scoped strictly to provider='ultrahuman' so other providers' genuine SDNN data is left
untouched. See scripts/data_migrations/relabel_ultrahuman_hrv_sdnn_to_rmssd.py.
"""
...
_SCRIPT_PATH = (
    Path(__file__).resolve().parents[2] / "scripts" / "data_migrations" / "relabel_ultrahuman_hrv_sdnn_to_rmssd.py"
)


def _load_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location("relabel_ultrahuman_hrv_sdnn_to_rmssd", _SCRIPT_PATH)
...
relabel_ultrahuman_hrv = _load_module().relabel_ultrahuman_hrv
...
def test_relabels_ultrahuman_sdnn_to_rmssd(db: Session) -> None:
    t = datetime(2026, 1, 1, 2, 0, tzinfo=timezone.utc)
    _make_hrv_point(db, provider=ProviderName.ULTRAHUMAN, type_id=SDNN_ID, recorded_at=t, value="55.0")
    _make_hrv_point(
        db, provider=ProviderName.ULTRAHUMAN, type_id=SDNN_ID, recorded_at=t + timedelta(minutes=5), value="60.0"
    )

    result = relabel_ultrahuman_hrv(db, dry_run=False)
...
def test_leaves_non_ultrahuman_sdnn_untouched(db: Session) -> None:
    t = datetime(2026, 1, 1, 2, 0, tzinfo=timezone.utc)
...
def test_dry_run_makes_no_changes(db: Session) -> None:
    ...
    result = relabel_ultrahuman_hrv(db, dry_run=True)
...
def test_idempotent_second_run_is_noop(db: Session) -> None:
    ...
    relabel_ultrahuman_hrv(db, dry_run=False)
    second = relabel_ultrahuman_hrv(db, dry_run=False)
...
def test_relabels_archive_table(db: Session) -> None:
    ...
    _make_archive_row(db, provider=ProviderName.ULTRAHUMAN, type_id=SDNN_ID, bucket_start_at=t)
