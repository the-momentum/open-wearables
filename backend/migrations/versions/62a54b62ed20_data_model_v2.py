...
def upgrade() -> None:
    ...
    # === data_point_series: Add new columns as NULLABLE first ===
    op.add_column("data_point_series", sa.Column("external_id", sa.String(length=100), nullable=True))
    op.add_column("data_point_series", sa.Column("external_device_mapping_id", sa.UUID(), nullable=True))
    op.add_column("data_point_series", sa.Column("series_type_definition_id", sa.Integer(), nullable=True))

    # Copy data from old columns to new columns
    op.execute("UPDATE data_point_series SET external_device_mapping_id = external_mapping_id")
    op.execute("UPDATE data_point_series SET series_type_definition_id = series_type_id")

    # Now set columns to NOT NULL
    op.alter_column("data_point_series", "external_device_mapping_id", nullable=False)
    op.alter_column("data_point_series", "series_type_definition_id", nullable=False)
    ...
    # === event_record: Add new column as NULLABLE first ===
    op.add_column("event_record", sa.Column("external_id", sa.String(length=100), nullable=True))
    op.add_column("event_record", sa.Column("external_device_mapping_id", sa.UUID(), nullable=True))

    # Copy data from old column to new column
    op.execute("UPDATE event_record SET external_device_mapping_id = external_mapping_id")

    # Now set column to NOT NULL
    op.alter_column("event_record", "external_device_mapping_id", nullable=False)
    ...
    # === external_device_mapping: Add new column as NULLABLE first ===
    op.add_column("external_device_mapping", sa.Column("provider_name", sa.String(length=10), nullable=True))

    # Copy data from old column to new column
    op.execute("UPDATE external_device_mapping SET provider_name = provider_id")

    # Now set column to NOT NULL
    op.alter_column("external_device_mapping", "provider_name", nullable=False)
...
def downgrade() -> None:
    ...
    # === external_device_mapping: Restore provider_id ===
    op.add_column(
        "external_device_mapping",
        sa.Column("provider_id", sa.VARCHAR(length=100), autoincrement=False, nullable=True),
    )
    op.execute("UPDATE external_device_mapping SET provider_id = provider_name")
    ...
    # === event_record: Restore external_mapping_id ===
    op.add_column("event_record", sa.Column("external_mapping_id", sa.UUID(), autoincrement=False, nullable=True))
    op.execute("UPDATE event_record SET external_mapping_id = external_device_mapping_id")
    op.alter_column("event_record", "external_mapping_id", nullable=False)
    ...
    # === data_point_series: Restore old columns ===
    op.add_column("data_point_series", sa.Column("series_type_id", sa.INTEGER(), autoincrement=False, nullable=True))
    op.add_column("data_point_series", sa.Column("external_mapping_id", sa.UUID(), autoincrement=False, nullable=True))
    op.execute("UPDATE data_point_series SET external_mapping_id = external_device_mapping_id")
    op.execute("UPDATE data_point_series SET series_type_id = series_type_definition_id")
    op.alter_column("data_point_series", "external_mapping_id", nullable=False)
