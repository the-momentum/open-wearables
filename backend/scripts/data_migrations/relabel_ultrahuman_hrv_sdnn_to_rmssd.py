#!/usr/bin/env python3
"""Relabel Ultrahuman HRV series stored as SDNN to RMSSD.

Ultrahuman's ring calculates HRV with the RMSSD method (confirmed by Ultrahuman
support), but the ingestion mapped the ``hrv`` samples to the SDNN series type
(id=3) instead of RMSSD (id=7). The values themselves are correct — only the label
is wrong — so this is a pure relabel, scoped strictly to provider='ultrahuman' so
other providers' genuine SDNN data (e.g. Apple Watch) is left untouched. The
mapping was fixed in ultrahuman/coverage.py in the same change; this script
corrects pre-existing rows in data_point_series and data_point_series_archive.

The same measurements can also reach us through Apple Health, where the Ultrahuman
app writes them into HealthKit's only HRV type, SDNN. Those rows arrive under
provider='apple' and keep the label Apple gives them, so the provider scope here
...
(the RMSSD row already holds the same Ultrahuman value) rather than relabeled.
...
Usage (inside Docker):
    docker compose exec app uv run python scripts/data_migrations/relabel_ultrahuman_hrv_sdnn_to_rmssd.py --dry-run
    docker compose exec app uv run python scripts/data_migrations/relabel_ultrahuman_hrv_sdnn_to_rmssd.py
"""
...
# Any SDNN Ultrahuman rows still present collided with an existing RMSSD row above.
...
def relabel_ultrahuman_hrv(db: Session, *, dry_run: bool) -> dict[str, int]:
    """Relabel Ultrahuman HRV SDNN rows to RMSSD. Does not commit — caller owns the transaction.
...
def main(dry_run: bool) -> None:
    with SessionLocal() as db:
        result = relabel_ultrahuman_hrv(db, dry_run=dry_run)
        if dry_run:
            return
        if not any(result.values()):
            print("Nothing to do — no Ultrahuman SDNN rows found.")
            return
        db.commit()
        print("Done.")
