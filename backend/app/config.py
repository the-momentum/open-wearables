...
class Settings(BaseSettings):
    ...
    strava_webhook_path_secret: SecretStr | None = None  # appended to callback URL as ?token=; derived from verify_token if unset
    strava_webhook_signature_tolerance_seconds: int = 300
    ...
    @model_validator(mode="after")
    def derive_strava_webhook_path_secret(self) -> "Settings":
        if self.strava_webhook_path_secret is None or self.strava_webhook_path_secret.get_secret_value() == "":
            self.strava_webhook_path_secret = self.strava_webhook_verify_token
        return self
