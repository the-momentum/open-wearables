...
class TestBaseToDictDataPointSeries:
    ...
    def test_base_to_dict_data_point_series_timestamp(self, db):
        ...
        # Arrange
        mapping = create_external_device_mapping(db)
        data_point = create_data_point_series(db, mapping=mapping)
...
class TestBaseToDictIntegration:
    ...
    def test_convert_multiple_related_models(self, db):
        ...
        event = create_event_record(db, mapping=mapping)
    ...
    def test_convert_models_with_same_user(self, db):
        ...
        # Arrange
        user = create_user(db)
        mapping1 = create_external_device_mapping(db, user=user, device_id="device-1")
        mapping2 = create_external_device_mapping(db, user=user, device_id="device-2")
