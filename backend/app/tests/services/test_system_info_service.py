...
class TestSystemInfoServiceGetSystemInfo:
    ...
    def test_get_system_info_weekly_growth_users(self, db: Session):
        ...
        create_user(db, created_at=two_weeks_ago - timedelta(days=1))  # Before tracking period
        create_user(db, created_at=two_weeks_ago + timedelta(days=1))  # Last week
        create_user(db, created_at=week_ago + timedelta(days=1))  # This week
        create_user(db, created_at=now - timedelta(hours=1))  # This week
    ...
    def test_get_system_info_handles_null_workout_type(self, db: Session):
        ...
        create_event_record(db, mapping=mapping, category="workout", type_=None)
        create_event_record(db, mapping=mapping, category="workout", type_=None)
