...
class TestUserServiceGetCountInRange:
    ...
    def test_get_count_in_range_with_users(self, db: Session) -> None:
        ...
        create_user(db, created_at=two_weeks_ago)  # Outside range
        create_user(db, created_at=week_ago + timedelta(days=1))  # Inside range
        create_user(db, created_at=now - timedelta(days=1))  # Inside range
    ...
    def test_get_count_in_range_empty_result(self, db: Session) -> None:
        ...
        create_user(db)  # Created now, outside future range
