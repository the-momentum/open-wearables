...
@shared_task
def refresh_dashboard_total_data_points() -> None:
    ...
    try:
        ...
        # Release the lock only on success; on failure leave it to expire so the lock's TTL acts as
        # a backoff instead of re-running the expensive recount on every subsequent dashboard load.
        dashboard_stats_cache.release_refresh_lock()
