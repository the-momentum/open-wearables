...
description: "Generate synthetic users, workouts, sleep, meals, and time series from the dashboard. Reproducible via a random seed embedded in the user name."
...
"og:url": "https://docs.openwearables.io/developer-portal/settings/seed-data-generator"
"og:type": "article"
...
The Seed Data Generator creates synthetic users with configurable workouts, sleep records, meals, and time series. Use it to populate a fresh instance for demos, exercise dashboards and API integrations, or reproduce a specific dataset that a teammate shared.
...
| **Comprehensive** | 150 workouts, 60 sleeps, 90 meals, 5 providers, 80% time series. Large dataset. |
| **Nutrition Focused** | 180 meals (3/day), 10 light workouts, no sleep. |
...
Off by default - opt in explicitly (existing presets stay meal-free unless they enable it). Each meal is an `EventRecord` (category `meal`) plus a `MealDetails` row and correlated nutrient samples (calories, protein, carbs, fat, fiber, hydration) linked back to it, mirroring how real HealthKit/Health Connect meal correlations are stored. Configure:
- **Meal count** (0-1000), **Meal types** - pick from breakfast/lunch/dinner/snack or leave empty for random.
- **Calories range** - per-meal calorie total; macros are derived from it with a randomized protein/fat/carb split.
- **Date range** (from/to).
