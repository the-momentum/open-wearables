...
<Note>
If `RESEND_API_KEY` or `EMAIL_FROM_ADDRESS` is missing, the invitation stays **pending** and the backend logs a warning instead of sending. Use **Copy invite link** on the pending invitation and share the link manually.
</Note>
