---
title: "Open Wearables Platform Architecture"
...
**Core Framework:**
- **FastAPI** (Python 3.14+) - Modern, fast web framework with automatic OpenAPI documentation
...
The platform is designed for **self-hosting** with Docker. For production, use the published images (see [Deploying with Docker](/deployment/docker)); the repository's `docker-compose.yml` runs the same stack locally for development:
...
- `frontend` - React frontend (TanStack Start, served by Nitro)
