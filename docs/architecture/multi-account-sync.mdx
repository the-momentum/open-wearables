...
Garmin sends a single webhook payload for one `userId` (their internal account ID). The platform fans out unconditionally:

1. `get_all_by_provider_user_id("garmin", garmin_user_id)` returns all active OW connections sharing that Garmin account.
2. The activity or wellness data is **parsed once**, then saved independently for every connected profile.
3. The **first** connection in the result set is treated as *primary* and receives a `WEBHOOK` sync status event. All others receive a `LINKED_ACCOUNT` event with `primary_user_id` pointing at the primary profile.

No distributed lock is needed here — Garmin sends the webhook exactly once, and saving the same record under N user IDs is idempotent (duplicate protection is handled at the DB layer via `IntegrityError` detection).
...
Secondary profiles receive their data through the **webhook fan-out** described above — every webhook the primary triggers delivers data to all linked profiles.
...
1. `try_become_primary(provider, provider_user_id, user_id, scope="pull")` is called.
   - **Won** → performs the API call normally; releases the lock after completion (or on error).
