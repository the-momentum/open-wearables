...
Dietary samples aggregated per day (SUM). Currently populated from Apple HealthKit
(`HKQuantityTypeIdentifierDietary*`). Android Health Connect support is deferred
until the SDK surfaces per-nutrient identifiers.

| Type | Unit | Description |
|------|------|-------------|
| `dietary_energy` | kcal | Calories consumed |
| `dietary_protein` | g | Protein consumed |
| `dietary_carbohydrates` | g | Total carbohydrates consumed |
| `dietary_fat_total` | g | Total fat consumed |
| `dietary_fat_saturated` | g | Saturated fat consumed |
| `dietary_fat_monounsaturated` | g | Monounsaturated fat consumed |
| `dietary_fat_polyunsaturated` | g | Polyunsaturated fat consumed |
| `dietary_fiber` | g | Dietary fiber consumed |
| `dietary_sugar` | g | Sugar consumed |
| `dietary_sodium` | g | Sodium consumed (HKUnit-native gram; display layers typically format as mg) |
| `dietary_cholesterol` | g | Cholesterol consumed (HKUnit-native gram; display layers typically format as mg) |
| `dietary_caffeine` | g | Caffeine consumed (HKUnit-native gram; display layers typically format as mg) |
