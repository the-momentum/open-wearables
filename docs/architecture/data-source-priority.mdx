...
class DataSource(BaseDbModel):
    ...
    user_connection_id: Mapped[UUID | None]
    ...
    device_type: Mapped[DeviceType | None]  # watch, phone, band, scale
    software_version: Mapped[str_50 | None]
    manufacturer: Mapped[str_50 | None]
```

### 3. `PROVIDER_PRIORITY` (New)

Developer-configurable priority for data providers in `/summary` endpoints.

```python
class ProviderPriority(BaseDbModel):
    __tablename__ = "provider_priority"
    __table_args__ = (
        UniqueConstraint("application_id", "provider", name="uq_app_provider"),
    )
    
    id: Mapped[PrimaryKey[UUID]]
    application_id: Mapped[FKApplication]
    provider: Mapped[ProviderName]
...
### 4. `DeviceType` (Enum + System Priorities)
...
# System-wide device type priority (not a table, just config)
DEVICE_TYPE_PRIORITY: dict[DeviceType, int] = {
    DeviceType.WATCH: 1,
    DeviceType.BAND: 2,
    DeviceType.PHONE: 3,
    DeviceType.SCALE: 4,
...
erDiagram
    ...
    APPLICATION ||--o{ PROVIDER_PRIORITY : "configures"
...
| Scenario | provider | user_connection_id | source | original_source_name | device_model | device_type | manufacturer |
|----------|----------|-------------------|--------|---------------------|--------------|-------------|--------------|
| **Apple Watch via SDK** | `apple` | `conn-apple` | `apple_health_sdk` | `Apple Watch (Jan)` | `Watch7,2` | `watch` | `Apple Inc.` |
| **iPhone via SDK** | `apple` | `conn-apple` | `apple_health_sdk` | `iPhone` | `iPhone14,5` | `phone` | `Apple Inc.` |
| **Zepp Life → Apple Health SDK** | `apple` | `conn-apple` | `apple_health_sdk` | `Zepp Life` | `NULL` | `unknown` | `NULL` |
| **AutoSleep → Apple Health SDK** | `apple` | `conn-apple` | `apple_health_sdk` | `AutoSleep` | `NULL` | `unknown` | `NULL` |
| **Manual entry via SDK** | `apple` | `conn-apple` | `apple_health_sdk` | `Health` | `NULL` | `unknown` | `NULL` |
| **Auto Export app** | `apple` | `conn-apple-ae` | `apple_health_auto_export` | `NULL` | `NULL` | `unknown` | `NULL` |
| **Apple Health XML import** | `apple` | `NULL` | `apple_health_xml` | `Apple Watch (Jan)` | `Watch3,3` | `watch` | `Apple Inc.` |
| **XML import: Zepp Life data** | `apple` | `NULL` | `apple_health_xml` | `Zepp Life` | `NULL` | `unknown` | `NULL` |
| **Garmin Forerunner** | `garmin` | `conn-garmin` | `garmin_connect_api` | `NULL` | `Forerunner 265` | `watch` | `Garmin` |
| **Garmin Venu** | `garmin` | `conn-garmin` | `garmin_connect_api` | `NULL` | `Venu 3` | `watch` | `Garmin` |
| **Garmin Index Scale** | `garmin` | `conn-garmin` | `garmin_connect_api` | `NULL` | `Index S2` | `scale` | `Garmin` |
| **Garmin Vivosmart** | `garmin` | `conn-garmin` | `garmin_connect_api` | `NULL` | `Vivosmart 5` | `band` | `Garmin` |
| **Suunto Vertical** | `suunto` | `conn-suunto` | `suunto_api` | `NULL` | `Suunto Vertical` | `watch` | `Suunto` |
| **Polar Vantage** | `polar` | `conn-polar` | `polar_api` | `NULL` | `Vantage V3` | `watch` | `Polar` |
| **Whoop 4.0** | `whoop` | `conn-whoop` | `whoop_api` | `NULL` | `WHOOP 4.0` | `band` | `WHOOP` |
| **Oura Ring Gen3** | `oura` | `conn-oura` | `oura_api` | `NULL` | `Oura Ring Gen3` | `band` | `Oura` |
...
Developers configure priorities in the dashboard. Example:

| application_id | provider | priority | Description |
|----------------|----------|----------|-------------|
| `app-xyz` | `apple` | 1 | Apple has highest priority |
| `app-xyz` | `garmin` | 2 | Garmin as fallback |
| `app-xyz` | `oura` | 3 | Oura third |
| `app-xyz` | `polar` | 4 | Polar fourth |
| `app-xyz` | `whoop` | 5 | Whoop lowest |
...
├─────────────────────────────────────────────────────────────────┤
│  1. Get PROVIDER_PRIORITY for application_id                    │
│     → [apple:1, garmin:2, oura:3]                               │
│                                                                 │
...
1. **Provider priority** (app config): apple=1, garmin=2
2. **Select Apple** (has data, priority 1)
3. **Device type priority**: watch > phone > unknown
...
Device type is derived from:
1. **Apple**: Parse `product_type` (e.g., `Watch7,2` → `watch`, `iPhone14,5` → `phone`)
2. **Garmin**: Match device model keywords (`Forerunner` → `watch`, `Vivosmart` → `band`, `Index` → `scale`)
3. **Fallback**: Use heuristics from `original_source_name` (e.g., "Mi Band" → `band`)
...
| **SDK + Auto Export** | Allow both, deduplicate | Users may have both; system handles duplicates transparently |
| **Multiple accounts per provider** | Not allowed | Constraint `(user_id, provider)` - simplifies model and UI |
| **Partial data** | Fallback to complete data | If priority provider has <80% coverage, use next provider |
...
- Apple Health Auto Export (batch export app)
...
Implement **coverage-based fallback** with a configurable threshold (default: 80%):

```
┌─────────────────────────────────────────────────────────────────┐
│              Partial Data Fallback Algorithm                     │
├─────────────────────────────────────────────────────────────────┤
...
```python
def calculate_coverage(user_id: UUID, provider: str, date: date) -> float:
    """Calculate data coverage percentage for a provider on a given day."""

    # For time-series data (steps, HR): check hourly buckets
    hours_with_data = count_distinct_hours_with_data(user_id, provider, date)
    coverage = hours_with_data / 24.0

    # For event data (sleep, workouts): check if primary event exists
    # Sleep: at least one sleep record covering night
    # Workout: not applicable (workouts are discrete events)

    return coverage

# Configuration
MIN_DATA_COVERAGE_THRESHOLD = 0.8  # 80%
```
...
Result: Use Garmin data (meets 80% threshold)
```
...
The unique constraint `(user_id, device_model, source)` needs special handling for NULLs.
...
INSERT INTO data_source (user_id, device_model, source)
VALUES ('user-1', NULL, 'apple_health_sdk');
...
Use `COALESCE` in the constraint or a partial unique index:

```sql
-- Option A: Functional unique index with COALESCE
CREATE UNIQUE INDEX uq_data_source_identity
ON data_source (user_id, COALESCE(device_model, ''), source);

-- Option B: Partial index for NULL case
CREATE UNIQUE INDEX uq_data_source_null_device
ON data_source (user_id, source)
WHERE device_model IS NULL;
```

### Device Type Inference

When `device_model` is NULL, attempt to infer `device_type` from `original_source_name`:

```python
ORIGINAL_SOURCE_DEVICE_TYPE_MAP = {
    "Mi Band": DeviceType.BAND,
    "Xiaomi Mi Band": DeviceType.BAND,
    "Amazfit Band": DeviceType.BAND,
    "Zepp Life": DeviceType.UNKNOWN,  # Could be watch or band
    "AutoSleep": DeviceType.WATCH,    # iOS-only, requires Apple Watch
    "Health": DeviceType.UNKNOWN,     # Manual entry
}

def infer_device_type(original_source_name: str | None) -> DeviceType:
    if not original_source_name:
        return DeviceType.UNKNOWN

    for pattern, device_type in ORIGINAL_SOURCE_DEVICE_TYPE_MAP.items():
        if pattern.lower() in original_source_name.lower():
            return device_type
...
1. **Per-metric priority** - Different provider rankings for sleep vs. activity vs. body metrics
2. **Data quality scoring** - Weight measurements from scale > manual entry
3. **Aggregator chain tracking** - Track full path: Device → App → Apple Health → Our system
4. **Automatic duplicate detection** - Flag overlapping workouts for user review
