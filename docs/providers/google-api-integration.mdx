...
| [OAuth app verification](https://developers.google.com/identity/protocols/oauth2/production-readiness/restricted-scope-verification) | Google Trust and Safety | free | 3-5 business days for the scope review itself |
...
<Warning>
Plan **6-10 weeks** end to end for the first pass, and put a reminder in the calendar a month before each anniversary. The assessment is annual — letting it lapse pulls your restricted scopes, which breaks every connected user at once.
</Warning>
...
<Note>
Verification is per OAuth client, so keep a separate Google Cloud project for staging. That one stays in Testing and is never reviewed; only the production client goes through the process.
</Note>
