...
| **Webhooks (push)** | Oura notifies Open Wearables in real time when sleep, readiness, activity, SpO2, or workout data is created or updated. Subscriptions are app-level and are registered automatically when you switch Oura to webhook live-sync mode. |
...
- **Webhook mode** enabled (optional, for real-time updates) - subscriptions are registered automatically
...
```
  ...
  <Step title="Enable webhooks for real-time updates (optional)">
    ...
    **Subscribed data types:** `sleep`, `daily_sleep`, `daily_readiness`, `daily_activity`, `daily_spo2`, `daily_cardiovascular_age`, `workout` - each for the `create` and `update` events.
    ...
    <Accordion title="Prefer the API? Switch modes programmatically">
    The dashboard toggle calls the same endpoint you can hit directly:

    ```bash
    curl -X PUT "http://localhost:8000/api/v1/oauth/providers/oura" \
      -H "X-Open-Wearables-API-Key: YOUR_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"live_sync_mode": "webhook"}'
    ```

    Verify the subscriptions were registered:

    ```bash
    curl -X GET "http://localhost:8000/api/v1/providers/oura/webhooks/subscriptions" \
      -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
    ```
    </Accordion>
