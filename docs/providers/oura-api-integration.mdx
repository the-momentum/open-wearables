...
Oura provides access to sleep, readiness, activity, heart rate, and workout data through the Oura API v2. The integration uses OAuth 2.0 for authentication, pull-based syncing to fetch daily metrics, and webhooks for real-time updates.
...
| **Webhooks (push)** | Oura notifies Open Wearables in real time when sleep, readiness, activity, SpO2, or workout data is created or updated. Subscriptions are app-level and can be registered programmatically via the Open Wearables API. |
...
- **Webhook subscriptions** registered (optional, for real-time updates)
...
<Steps>
  ...
  <Step title="Create an OAuth application">
    ...
    - **Redirect URIs**: Your OAuth callback URL. Oura requires a publicly accessible **HTTPS** URL, so plain `http://localhost` is not accepted. For local development, expose your backend with [ngrok](/dev-guides/ngrok-setup) and use e.g. `https://your-subdomain.ngrok-free.app/api/v1/oauth/oura/callback`. In production use your public domain, e.g. `https://your-domain/api/v1/oauth/oura/callback`.
...
OURA_REDIRECT_URI=https://your-subdomain.ngrok-free.app/api/v1/oauth/oura/callback
OURA_DEFAULT_SCOPE=personal daily activity heartrate workout session spo2 ring_configuration heart_health
# Optional: token Oura echoes when verifying your webhook subscription.
# Defaults to your SECRET_KEY if left unset.
...
```
    ...
    **Configuration details:**
    ...
    | `OURA_REDIRECT_URI` | Must match a redirect URI registered in the Developer portal, and must be a public HTTPS URL. For local dev, use your [ngrok](/dev-guides/ngrok-setup) URL, e.g. `https://your-subdomain.ngrok-free.app/api/v1/oauth/oura/callback` |
    ...
    | `OURA_WEBHOOK_VERIFICATION_TOKEN` | Token used to verify webhook subscriptions. Optional - defaults to your `SECRET_KEY` if unset |
  ...
  <Step title="Connect a user via OAuth">
    ...
    **Response:**
    ```json
    {
      "authorization_url": "https://cloud.ouraring.com/oauth/authorize?client_id=...&redirect_uri=...&response_type=code&scope=personal+daily+activity+heartrate+workout+session+spo2+ring_configuration+heart_health&state=...",
      "state": "abc123..."
    }
    ```
    ...
    **3. Oura redirects back** to the callback URI configured in your `.env` (`OURA_REDIRECT_URI`). Open Wearables automatically exchanges the authorization code for access and refresh tokens.
    ...
    **4. Verify the connection:**
    ...
    <Note>
      The `redirect_uri` parameter in the authorize call is where the **user** is sent after the flow completes (e.g., back to your app). This is separate from `OURA_REDIRECT_URI` in your `.env`, which is the **server-side** OAuth callback that Oura sends the authorization code to.
    </Note>
  </Step>

  <Step title="Set up webhooks (optional)">
    Webhooks give you real-time updates instead of waiting for the next polling cycle. When Oura records new sleep, readiness, activity, SpO2, or workout data, it immediately notifies Open Wearables, which fetches and saves the record.

    Oura webhook subscriptions are **app-level** (they cover all authorized users) and can be registered programmatically. Make sure your `OURA_CLIENT_ID` and `OURA_CLIENT_SECRET` are set first.

    **1. Register subscriptions:**

    ```bash
    curl -X POST "http://localhost:8000/api/v1/oura/webhooks/subscriptions?callback_url=https://your-domain/api/v1/oura/webhooks" \
      -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
    ```

    For local development, use a tunnel such as [ngrok](/dev-guides/ngrok-setup) as the `callback_url`:

    ```
    https://your-subdomain.ngrok-free.app/api/v1/oura/webhooks
    ```

    Open Wearables registers a subscription for every supported data type and event type, and Oura verifies each one using your `OURA_WEBHOOK_VERIFICATION_TOKEN`.

    **2. List active subscriptions:**

    ```bash
    curl -X GET "http://localhost:8000/api/v1/oura/webhooks/subscriptions" \
      -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
    ```
    ...
    **Subscribed data types:** `sleep`, `daily_sleep`, `daily_readiness`, `daily_activity`, `daily_spo2`, `daily_cardiovascular_age`, `workout` - each for the `create` and `update` events.
    ...
    <Note>
    Oura webhook subscriptions expire and are renewed automatically by a scheduled task. You can also renew them manually via `POST /api/v1/oura/webhooks/subscriptions/renew`.
    </Note>
  </Step>
...
| **Webhook callback** | `POST /api/v1/oura/webhooks` |
