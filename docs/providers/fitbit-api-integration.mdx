...
```
    ...
    **Configuration details:**
    ...
    | `FITBIT_REDIRECT_URI` | Must match the Redirect URL registered in your Fitbit application. For local dev: `http://localhost:8000/api/v1/oauth/fitbit/callback` |

    <Note>
    The OAuth scopes used are `activity heartrate sleep profile`. These grant read access to activity/workout data, heart rate data, sleep data, and basic profile information. Only activity data is actively synced at this time.
    </Note>
  </Step>
  ...
  <Step title="Sync data">
    ...
    ```bash
    # Sync all data types
    curl -X POST "http://localhost:8000/api/v1/providers/fitbit/users/{user_id}/sync?data_type=all" \
      -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
    ```
