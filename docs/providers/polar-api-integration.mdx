...
| Laps, splits, pool lengths | Yes (parsed from the exercise FIT file) |
...
| **Polling (pull) — Exercise FIT** | For each saved exercise, Open Wearables also pulls the device's own recording ([`/v3/exercises/{id}/fit`](https://www.polar.com/accesslink-api/#get-exercise-fit)). Laps, splits and pool lengths go to `workout_details.segments`, and time-in-zone to `hr_zones` / `power_zones` when the device recorded it. The file's per-second samples are written to `data_point_series` only when `INGEST_WORKOUT_SAMPLES` is enabled (off by default). Exercises without a FIT file (manual entries, third-party uploads) keep their summary. |
...
### Reading workout detail

`GET /api/v1/users/{user_id}/events/workouts` includes nullable `segments`,
`hr_zones`, and `power_zones` fields on each workout. These expose stored FIT detail
for Polar and other providers, including Garmin. `segments` contains the recorded
lap, split, or pool-length objects; `hr_zones` contains `zones` and optional
`max_hr` / `threshold_hr`; `power_zones` contains `zones` and optional `ftp_watts`.
Unavailable fields are `null`; no per-second sample ingestion is required.

Polar's `TREADMILL_RUNNING`, `POOL_SWIMMING`, and `STRENGTH_TRAINING` sport details
map to `treadmill`, `pool_swimming`, and `strength_training`. `HIIT` maps to the
shared `cardio_training` category. `OTHER_INDOOR` remains `generic` because Polar
does not identify a more specific activity. Heart-rate summaries retain Polar's
reported values. Existing records retain their original classification until
explicitly reprocessed; duplicate sync does not replace stored workout types.
