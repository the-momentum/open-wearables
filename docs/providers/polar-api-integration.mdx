...
| Method | Description |
|--------|-------------|
| **Polling (pull) — Exercises** | Open Wearables fetches new exercises via the transaction-based AccessLink API (Celery). Covers exercises uploaded in the last 30 days. |
...
```
  ...
  <Step title="Sync data">
    ...
    <Note>
    Exercises: Polar only returns exercises uploaded within the last 30 days via the transaction-based endpoint. 24/7 data (sleep, activity, HR, etc.) is fetched over a 90-day lookback window using range endpoints.
    </Note>
  </Step>
