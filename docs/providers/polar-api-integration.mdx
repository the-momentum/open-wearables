...
| **Polling (pull) — Exercise FIT** | For each saved exercise, Open Wearables also pulls the device's own recording ([`/v3/exercises/{id}/fit`](https://www.polar.com/accesslink-api/#get-exercise-fit)) and stores its laps, splits and pool lengths in `workout_details.segments`. Exercises without a FIT file (manual entries, third-party uploads) keep their summary. |
