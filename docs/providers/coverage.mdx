...
<Accordion title="Withings">
  **Integration Method:** OAuth 2.0 + Webhooks (rest_pull + webhook_ping)
  ...
  - Body measures from `getmeas`: weight, height, lean body mass, body fat %, body fat mass, blood pressure (systolic/diastolic), heart rate, body temperature, skin temperature, oxygen saturation, skeletal muscle mass, hydration, VO2 max, cardiovascular age
  - Bone mass and pulse wave velocity are parsed but intentionally not yet mapped — they have no unified series type, pending agreement on adding new ones
