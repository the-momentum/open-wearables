...
<Accordion title="Withings">
  **Integration Method:** OAuth 2.0 + Webhooks (rest_pull + webhook_ping)
  ...
  - Webhook subscriptions are **per-user**: a `WithingsNotifyService` Celery task subscribes each user on OAuth connect (unlike app-level subscriptions used by most other providers)
