...
<Steps>
  ...
  <Step title="Your App + Mobile SDK" icon="mobile">
    The SDK reads data from the health store and handles background synchronization automatically. Available as native iOS (Swift), native Android (Kotlin), Flutter, or React Native.
  </Step>
...
The [Integration Guide](/sdk/integration) walks through the flow step by step, the same for every SDK.
...
For any real integration, use the **backend token flow**: your backend mints a short-lived, user-scoped token (write-only to the `/sdk/*` endpoints) via [`POST /api/v1/users/{user_id}/token`](/api-reference/external:-mobile-sdk/create-user-token) and forwards it to the app over its own authenticated API. Your `app_secret` never touches the device. This is the standard path, covered in the [Integration Guide](/sdk/integration).
...
In most SDKs `signOut()` only clears the credentials stored on the device. It makes no network call, so on its own the backend never learns the user left: the connection stays `active` behind a plausible `last_synced_at` and is indistinguishable from a healthy one. The native iOS SDK reports the disconnect itself from version 0.15.0; the other SDKs need the call below.

To report the disconnect, call [`DELETE /api/v1/users/{user_id}/connections/{provider}`](/api-reference/external:-connections/disconnect-provider-endpoint) with the SDK access token **before** signing out locally:
...
<CardGroup cols={2}>
  ...
  <Card title="React Native SDK" icon="arrow-right" href="/sdk/react-native">
