...
- **`syncDaysBack`** limits how much history is uploaded - from midnight that many days ago. Omitting it or passing `0` means **no limit**: the user's entire history. The smallest window is `1` (since yesterday). The value is persisted until sign-out. Widening it later requires `resetAnchors()` while sync is stopped.
