...
The Open Wearables Android SDK (`open_wearables_android_sdk`) is a native Kotlin SDK for secure background synchronization of health data from **Samsung Health** and **Health Connect** to the Open Wearables platform.
...
<Card title="GitHub Repository" icon="github" href="https://github.com/the-momentum/open_wearables_android_sdk">
...
Then add the `sdk` module in your app's `build.gradle.kts`:

```kotlin
dependencies {
    implementation("com.github.the-momentum.open_wearables_android_sdk:sdk:v0.12.0")
}
```
...
<Accordion title="Declared permissions">
  ...
  | `FOREGROUND_SERVICE_HEALTH` | Health foreground service type |
