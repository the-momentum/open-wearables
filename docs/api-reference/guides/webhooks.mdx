...
`start_time` / `end_time` are `null` when the ingestion run did not record time bounds (e.g. Apple Health XML imports). Fetch the actual samples via [`GET /api/v1/users/{user_id}/timeseries`](/api-reference/guides/quick-integration#step-4-fetch-health-data).
