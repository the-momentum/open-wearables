...
| `meal.created` | A new meal or nutrition-log entry was saved. |
...
```json
{
  ...
  "data": {
    ...
    "meal_type": "lunch",
    "start_time": "2025-12-19T12:30:00+01:00",
    "end_time": "2025-12-19T12:30:00+01:00",
    "zone_offset": "+01:00",
    "source": {
      "provider": "apple",
      "device": "iPhone"
...
<Note>
  `title` and `meal_type` come from the provider's food-log entry (or HealthKit/Health Connect correlation metadata) and are `null` when not reported. `macros` is `null` when none of its four fields were recorded for the meal; each of `calories_kcal` and `water_ml` is `null` independently when that nutrient wasn't reported.
</Note>
