...
All three accept the same `ApiKeyDep` authentication as the rest of the public API: pass your API key via the `X-Open-Wearables-API-Key` header.

<CodeGroup>
```bash X-Open-Wearables-API-Key
...
const es = new EventSource(url, {
  fetch: (input, init) =>
    fetch(input, { ...init, headers: { ...init.headers, 'X-Open-Wearables-API-Key': KEY } }),
});
