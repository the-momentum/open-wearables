...
<ParamField body="max_file_size" type="integer" default="52428800">
  Maximum file size in bytes (1KB - 5GB). Default is 50MB.
</ParamField>
...
<Tip>
  For large exports (multi-GB), set `expiration_seconds` to `3600` (the maximum) and `max_file_size` to the exact file size in bytes so the presigned URL stays valid for the full upload.
</Tip>

### Step 3: Start Processing

After the file lands in S3, queue the background import. The Open Wearables dashboard does this automatically.

Use this step when S3 event notifications (AWS SNS) are **not** configured — for example on S3-compatible storage such as Railway Object Storage.

```bash
curl -X POST "https://api.example.com/api/v1/users/{user_id}/import/apple/xml/s3/process" \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{"file_key": "user-uuid/raw/export.xml"}'
```

<ParamField body="file_key" type="string" required>
  The `file_key` returned from the presigned URL step. Must belong to the target user (`{user_id}/...`).
</ParamField>
...
```json 202 Accepted
{
  ...
  "file_key": "3fa85f64-5717-4562-b3fc-2c963f66afa6/raw/export.xml",
  "bucket": "my-bucket",
...
<Note>
  When AWS SNS is configured, S3 upload events trigger processing automatically and this step is optional. Calling it twice queues two import tasks — only use it when SNS is unavailable or as a manual retry.
</Note>
  ...
  <Tab title="S3 Presigned URL (Recommended)">
    <Steps>
      ...
      <Step title="Start Processing">
        **With AWS SNS:** S3 upload events trigger `process_aws_upload` automatically.

        **Without SNS (e.g. Railway Object Storage):** Call `POST /users/{user_id}/import/apple/xml/s3/process` with the `file_key` from step 1. The Open Wearables dashboard does this automatically after upload.
      </Step>
    </Steps>
    ...
    def main():
        ...
        # Step 4: Queue processing (required without AWS SNS)
        process_response = requests.post(
            f"{API_URL}/api/v1/users/{USER_ID}/import/apple/xml/s3/process",
            headers=headers,
            json={"file_key": presigned_data["file_key"]},
        )
        process_response.raise_for_status()
        task_id = process_response.json()["task_id"]
        print(f"✓ Import queued (task_id: {task_id})")
