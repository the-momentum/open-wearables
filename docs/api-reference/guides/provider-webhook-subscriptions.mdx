...
Supported `{provider}` values: `polar`, `oura`, `strava`. Withings subscriptions are managed per connected user.
...
| **Withings** | 1 per user and notification category | HEAD reachability probe | `WITHINGS_WEBHOOK_TOKEN` query parameter |
...
| `HEAD` | `/api/v1/providers/{provider}/webhooks` | Handles callback reachability probes (Withings). |
...
Withings recommends adding a random shared secret to the exact callback URL. Set `WITHINGS_WEBHOOK_TOKEN` to a value generated with `openssl rand -hex 32`; Open Wearables registers `?token=<secret>` and validates it on notification POSTs and reachability HEAD probes. See the [Withings notification security guidance](https://developer.withings.com/developer-guide/v3/integration-guide/public-health-data-api/data-api/notifications/notification-overview/).

Withings also recommends allowing its published notification IP ranges at the network edge. Those ranges are distributed through the DNS records documented in the same security guidance and should be monitored for changes by the deployment operator.
