...
| `limit` | 50 | Items per page (max: 1000 for timeseries, 100 for workouts and sleep) |

Use the `next_cursor` from the response to fetch the next page.

<Note>
  A timeseries request with a `resolution` other than `raw` pages by bucket, and `total_count` is
  `null` — counting buckets would require scanning the whole requested range. Cursors are not
  interchangeable between resolutions or endpoints.
</Note>
...
| Date-only | `2023-11-07` | Start bounds: midnight UTC (`2023-11-07T00:00:00Z`). End bounds: end of day (`2023-11-07T23:59:59.999999Z`) |
| Unix timestamp (seconds) | `1699332716` | UTC |

<Note>
  Date-only ranges are inclusive of both boundary days: a date-only **start date** normalizes to midnight and a date-only **end date** to the end of that day, so `start_date=2023-11-07&end_date=2023-11-07` covers the whole of November 7th. Values with an explicit time component are used exactly as given.
</Note>
