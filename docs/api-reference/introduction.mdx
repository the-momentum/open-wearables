...
Errors are returned as [RFC 9457](https://www.rfc-editor.org/rfc/rfc9457) problem details objects with the `application/problem+json` media type. The `code` field is a stable machine-readable identifier; validation errors (`422`) add an `errors` array. See the [error handling guide](/api-reference/guides/error-handling) for the full reference.
