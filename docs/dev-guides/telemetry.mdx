  ...
  "endpoint_usage": {
    ...
    "routes": {
      "GET /api/v1/users/{user_id}/timeseries": {
        "api_key": { "2xx": "1k-10k", "4xx": "1-10" },
        "developer_jwt": { "2xx": "11-100" }
      },
      "GET /api/v1/dashboard/stats": { "developer_jwt": { "2xx": "11-100" } },
      "POST /api/v1/providers/garmin/webhooks": { "none": { "2xx": "101-1k" } },
      "unmatched": { "none": { "4xx": "11-100" } }
    }
  }
}
```
...
This fully disables telemetry: no scheduled task, no startup ping, no HTTP calls, no request
counting.
...
`telemetry.mntm.dev` in your firewall. Telemetry is best-effort, so a blocked request is logged at
