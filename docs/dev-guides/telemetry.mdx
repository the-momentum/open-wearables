...
| `data_granularity` | Per-provider override of 24/7 data granularity: `raw`, `hourly` or `daily`, or `null` when not set and the instance default applies. Only Google Health reads it at the moment | `null` | Whether anyone uses the rollup modes |
...
| `default_data_granularity` | Instance-wide default granularity: `raw`, `hourly` or `daily`. Only Google Health reads it at the moment | `raw` | Whether the rollup modes are used |
