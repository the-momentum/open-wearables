...
<Tabs>
  ...
  <Tab title="Workout">
    ...
    reporting provider - `entry_source` is one of `manual` / `automatic` / `unknown` (`manual`
    only for hand-entered/self-reported data with no device measurements; a file like FIT/GPX
    uploaded from another device is still real measured data, so it counts as `automatic`),
