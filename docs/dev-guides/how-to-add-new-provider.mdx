...
class MyProviderStrategy(BaseProviderStrategy):
    def __init__(self) -> None:
        super().__init__()
        self.webhook_service = my_provider_webhook_service
...
When an admin switches the sync mode to `webhook` via `PUT /api/v1/oauth/providers/{provider}`, the platform dispatches `register_provider_webhooks`. For application-scoped providers, the task calls `strategy.webhook_service.register_subscriptions(callback_url)` in the background. The callback URL is derived from `API_BASE_URL`: `{API_BASE_URL}/api/v1/providers/{provider}/webhooks`.
