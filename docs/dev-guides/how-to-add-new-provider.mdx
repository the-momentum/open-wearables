...
1. Make `register_subscriptions(callback_url)` dispatch one `register_user_webhooks` task per active connection (`UserConnectionRepository.get_all_active_by_provider`) instead of registering anything itself. The `callback_url` argument is unused — each per-user task builds its own.
2. Implement `register_user_subscriptions(db, user_id)` on your own webhook service — it stays provider-local rather than going on `BaseWebhookService`, and the task resolves it dynamically. That is what the per-user task calls; work against the provider's currently configured live-sync mode (`ProviderSettingsRepository.get_live_sync_mode`), listing the provider's existing subscriptions first and changing only the gap, so a redelivered task is safe and a mode of `pull` revokes rather than creates.
...
Because each connection holds its own subscription, both `register_subscriptions()` and `deregister_subscriptions()` fan out the same way — the per-user task reads the configured mode and revokes when it is `pull`, so neither direction is special-cased in shared code. `app/services/providers/withings/webhook_service.py` is the reference implementation.
