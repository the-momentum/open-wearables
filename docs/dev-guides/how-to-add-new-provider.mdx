...
Implement `deregister_subscriptions()` on your webhook service on top of the list/get/delete methods it already has — that is the `pull` half, and without it no events stop arriving when an admin switches back to periodic pull. Override `apply_live_sync_mode()` on the strategy only if reconciliation cannot go through that task at all.
