...
|---|---|---|
| `rest_pull` | Periodic REST API polling (OAuth) | Garmin, Oura, Whoop, Strava, Suunto |
...
class SuuntoWebhookHandler(BaseWebhookHandler):
    ...
    def dispatch(self, db: DbSession, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        event_type = payload.get("workoutKey") and "WORKOUT_CREATED" or payload.get("event_type")
