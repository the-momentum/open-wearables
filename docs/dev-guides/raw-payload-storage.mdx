...
- **Oversized payloads** - Payloads exceeding `RAW_PAYLOAD_MAX_SIZE_BYTES` are silently skipped with a warning log. Data processing continues normally.
...
- **Missing boto3** - If the `boto3` library is not installed, S3 backend falls back to `disabled`.
